...
## ★ 최신 업데이트 (2026-06-01) — v2 재학습이 전통 모델을 능가
...
Drive 내부 복사(상속 접근)해 해결. **라이브 실측 AUC 0.763 / KS 0.420 (100건)** —
로지스틱(0.697)·XGBoost(0.723) **모두 능가**(v1 r=8 은 0.640 이었음). 부실확률 분포
0.04~0.55 로 변별력 대폭 향상. 프론트엔드도 전면 재디자인("정밀 신용 계기판") 후 라이브 반영.
