...
[![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/hwkim0527/CSS_rating2/blob/main/notebooks/train_llm_colab.ipynb)
...
1. 위 버튼 클릭 → Colab 열림
2. **런타임 → 런타임 유형 변경 → T4 GPU** 선택 (무료)
3. **모든 셀 실행** (`Ctrl+F9` 또는 셀별 `Shift+Enter`)
4. GitHub Personal Access Token 1회 입력 (비공개 저장소 클론용)
5. 약 **1.5~2시간 후** `artifacts/metrics.json`이 자동 갱신되어 저장소에 푸시

**학습 사양 (효율 모드)**:
- 모델: Qwen3-14B (Apache 2.0)
- 양자화: 4-bit NF4 (QLoRA)
- LoRA: r=8, alpha=16
- 데이터: 10,000 샘플 × 1 epoch
- 시퀀스 길이: 512
- 비용: **$0** (Colab 무료 티어)
