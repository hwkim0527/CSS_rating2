...
> **최종 Qwen3-14B sLLM**(50k · LoRA r=16)이 운영 시스템의 채점 모델로 확정되었습니다.
> 전통 모델(로지스틱 0.697)을 능가하고, **XGBoost 와는 동일 100건 직접 비교 시 대등**
> (LLM 0.763 vs XGBoost 0.759, 통계적 동률). 표 데이터에서 sLLM 이 GBM 챔피언과 동률에
> 도달한 것 자체가 이례적입니다. sLLM 수치는 라이브 100건 실측(±0.05), 표의 LR/XGBoost
> 열은 전체 테스트셋(199k) 기준이라 직접 비교는 위 "동일 100건" 값으로. → `/compare` 페이지에서 실시간 확인.
