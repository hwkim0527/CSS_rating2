...
> **재학습(v2) sLLM이 v1(0.640) → 0.763 으로 대폭 향상**, 전통 모델(로지스틱 0.697)을 능가.
> XGBoost 와는 **동일 100건 직접 비교 시 대등**(LLM 0.763 vs XGBoost 0.759, 통계적 동률).
> 표 데이터에서 sLLM 이 GBM 챔피언과 동률에 도달한 것 자체가 이례적입니다.
> sLLM 수치는 라이브 100건 실측(±0.05), 표의 LR/XGBoost 열은 전체 테스트셋(199k) 기준이라
> sLLM 과 직접 비교는 위 "동일 100건" 값으로 봐야 합니다. v2 는 step-2500 체크포인트
> (학습을 끝까지 돌리면 추가 향상 여지). → `/compare` 페이지에서 실시간 확인.
...
│   │   ├── llm_finetune.py       # Qwen3-14B QLoRA (GCP)
...
| Qwen3-14B QLoRA | `artifacts/qwen3_lora/` | GCP에서 학습 후 산출 |
...
## 4. LLM 학습 (가장 효율적: Google Colab 무료 T4)
...
1. 위 버튼 클릭 → Colab 열림
2. **런타임 → 런타임 유형 변경 → GPU** 선택 (50k×3epoch 은 **L4/A100 권장**, 무료 T4 는 매우 느림)
3. **모든 셀 실행** (`Ctrl+F9` 또는 셀별 `Shift+Enter`)
4. 학습 완료 시 LoRA 어댑터가 Google Drive `Qwen3_fintech_v2` 폴더에 저장됨
   (배포의 `CSS_LLM_DRIVE_FOLDER_ID` 를 그 폴더 ID 로 교체하면 라이브 반영)
5. 평가 셀이 테스트셋 AUC/KS 를 측정해 `artifacts/metrics.json` 을 갱신

**학습 사양 (재학습 v2)**:
- 모델: Qwen3-14B (Apache 2.0)
- 양자화: 4-bit NF4 (QLoRA)
- LoRA: r=16, alpha=32
- 데이터: 50,000 샘플 × 3 epoch (`data/llm_seed/train_50k.jsonl.gz`)
- 시퀀스 길이: 512
- 비용: **$0** (Colab 무료 티어, 단 50k×3은 시간이 길어 L4/A100 권장)

학습이 끝나면 README 상단의 비교 표가 LLM 결과를 포함해 자동으로 갱신됩니다.

## 5. GCP Vertex AI 대안 (유료, 대규모 학습)
...
python -m src.training.llm_eval \
    --adapter_dir gs://${GCS_BUCKET}/css-rating2/qwen3_lora \
